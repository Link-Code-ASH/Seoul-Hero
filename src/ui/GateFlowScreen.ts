...
export function gateFlowScreen(step:GateEntryStep,meta:MetaState,draft:GateEntryDraft):string{
  let body='';
  if(step==='gateMap')body=`<div class="gate-card-grid map-grid">${Object.values(maps).map(map=>{const unlocked=isMapUnlocked(meta,map.id);return `<button class="gate-select-card map-card ${draft.mapId===map.id?'selected':''}" aria-pressed="${draft.mapId===map.id}" data-action="gate-map:${map.id}" ${unlocked?'':'disabled'}><img src="${images[map.thumbnail].url}" alt=""><span><small>${unlocked?'작전 가능':'잠김'}</small><b>${escapeHtml(map.name)}</b></span></button>`}).join('')}</div>${next(step,!draft.mapId)}`;
  if(step==='gateDepth')body=depthStage(meta,draft);
  if(step==='gateCharacter')body=`<div class="gate-card-grid character-meta-grid">${Object.values(characters).map(character=>{const progress=meta.characters[character.id],unlocked=isCharacterUnlocked(meta,character.id),selected=draft.characterId===character.id;return `<button class="gate-select-card character-meta-card ${selected?'selected':''}" data-action="gate-character:${character.id}" aria-pressed="${selected}" ${unlocked?'':'disabled'}><span class="character-card-art"><img src="${images[character.portraitSprite??character.visual.sprite??'player'].url}" alt=""></span><span class="character-card-info"><small>AWAKENER · ${unlocked?'출동 가능':'잠김'}</small><b>${escapeHtml(character.name)}</b><em>돌파 ${progress?.breakthrough??0}</em><strong class="selection-seal">${selected?'작전 편성 완료':'각성자 선택'}</strong></span></button>`}).join('')}</div>${next(step,!draft.characterId)}`;
  if(step==='gateWeapon'){const choices=availableStartingWeapons(meta,draft.characterId);body=`<div class="gate-card-grid weapon-meta-grid">${choices.map(weapon=>{const selected=draft.startingWeaponId===weapon.id;return `<button class="gate-select-card weapon-meta-card ${selected?'selected':''}" data-action="gate-weapon:${weapon.id}" aria-pressed="${selected}"><span class="weapon-card-art">${weaponDetailArt(weapon.id)}</span><span class="weapon-card-info"><small>START Lv.1</small><b>${escapeHtml(weapon.name)}</b><strong class="selection-seal">${selected?'✓ 선택됨':'선택'}</strong></span></button>`}).join('')}</div>${next(step,!draft.startingWeaponId)}`;}
  if(step==='gateBlessing'){
    ...
    body=owned.length?`<div class="gate-card-grid blessing-meta-grid">${owned.map(value=>`<button class="gate-select-card blessing-meta-card ${draft.blessingId===value.id?'selected':''}" aria-pressed="${draft.blessingId===value.id}" data-action="gate-blessing:${value.id}"><img src="${images[value.image].url}" alt=""><span><small>Lv.${meta.blessings[value.id]!.level}</small><b>${escapeHtml(value.name)}</b><em>${escapeHtml(value.description)}</em></span></button>`).join('')}</div>${next(step,!draft.blessingId)}`:`<div class="no-blessing"><span>미해금</span><b>단군의 축복 없이 출동</b></div>${next(step)}`;
  }
  if(step==='gateConfirm'){const map=maps[draft.mapId],character=characters[draft.characterId],weapon=availableStartingWeapons(meta,draft.characterId).find(v=>v.id===draft.startingWeaponId),blessing=dangunBlessings[draft.blessingId];body=`<div class="deploy-summary"><div class="deploy-visual"><img class="deploy-map" src="${images[map?.thumbnail??'seoulIntersection'].url}" alt=""><span class="deploy-region"><small>OPERATION ZONE</small><b>${escapeHtml(map?.name??'-')}</b></span><img class="deploy-character" src="${images[character?.portraitSprite??character?.visual.sprite??'player'].url}" alt=""></div><div class="deploy-loadout"><div class="deploy-depth"><small>GATE DEPTH</small><b>${draft.gateDepth}</b><span>출동 편성 완료</span></div><div class="deploy-equipped"><div><img src="${images[character?.portraitSprite??character?.visual.sprite??'player'].url}" alt=""><span><small>각성자</small><b>${escapeHtml(character?.name??'-')}</b></span></div><div>${weapon?weaponDetailArt(weapon.id):''}<span><small>시작 무기</small><b>${escapeHtml(weapon?.name??'-')}</b></span></div><div><img src="${images[blessing?.image??'blessingCheonbuOath'].url}" alt=""><span><small>단군의 축복</small><b>${escapeHtml(blessing?.name??'없음')}</b></span></div></div></div></div>${button('gate-deploy','작전 시작 →','primary gate-deploy')}`;}
