...
export function gateFlowScreen(step:GateEntryStep,meta:MetaState,draft:GateEntryDraft):string{
  ...
  const map=maps[draft.mapId],character=characters[draft.characterId],weapon=availableStartingWeapons(meta,draft.characterId).find(v=>v.id===draft.startingWeaponId),blessing=dangunBlessings[draft.blessingId];
  const context=`<aside class="gate-context"><img src="${images[map?.thumbnail??'seoulIntersection'].url}" alt=""><span><small>구역</small><b>${escapeHtml(map?.name??'미선택')}</b></span><span><small>심도</small><b>${draft.gateDepth}</b></span><span><small>각성자</small><b>${escapeHtml(character?.name??'미선택')}</b></span><span><small>무기</small><b>${escapeHtml(weapon?.name??'미선택')}</b></span><span><small>축복</small><b>${escapeHtml(blessing?.name??'없음')}</b></span></aside>`;
  return `<section class="content-screen gate-flow-screen" style="--gate-bg:url('${images[map?.thumbnail??'seoulIntersection'].url}')">${stepHeader(step)}${weeklyCard(meta)}${context}<main>${body}</main></section>`;
}
