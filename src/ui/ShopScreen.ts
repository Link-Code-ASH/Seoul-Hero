...
export function shopScreen(run:RunState):string {
 ...
 return '<section class="level-modal shop-screen" tabindex="-1"><header class="shop-header"><h1>게이트<span>24</span></h1><div class="shop-wallet" aria-label="보유 마력석">'+price(Math.floor(run.runCurrency))+'</div></header><div class="shop-body">'+shopStats(run)+'<div class="shop-catalog"><section class="shop-section"><header><div><span class="shop-section-kicker">ARMORY</span><h2>무기</h2></div>'+button('shop-weapon-reroll','<span>↻</span> '+price(weaponCost),'refresh-stock',`aria-label="무기 교체 ${weaponCost} 마력석" ${weaponRerollDisabled?'disabled':''}`)+'</header><div class="shop-grid weapon-shop-grid">'+weaponCards+'</div></section><section class="shop-section"><header><div><span class="shop-section-kicker">SUPPLIES</span><h2>보조 장비</h2></div>'+button('shop-reroll','<span>↻</span> '+price(itemCost),'refresh-stock',`aria-label="보조 장비 교체 ${itemCost} 마력석" ${itemRerollDisabled?'disabled':''}`)+'</header><div class="shop-grid">'+itemCards+'</div></section><footer class="shop-footer">'+button('next-wave','전투 시작 <span>→</span>','primary next-combat',blocked?'disabled':'')+'</footer><details class="bag-summary"><summary>보유 아이템</summary>'+run.runItems.map(i=>'<p>'+escapeHtml(items[i.id]?.name??i.id)+' × '+i.count+'</p>').join('')+'</details></div></div>'+branchPicker(run)+'</section>';
}
