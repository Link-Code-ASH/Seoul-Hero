...
describe('paid weapon branch progression',()=>{
 ...
 it('removes the branch overlay as soon as the choice succeeds',()=>{const sim=shop(),run=sim.state,weapon=run.ownedWeapons[0]!,meta=createDefaultMeta();weapon.level=5;run.pendingBranchWeaponId=weapon.id;expect(shopScreen(run,meta)).toContain('shop-branch-overlay');expect(sim.chooseBranch('A')).toBe(true);expect(shopScreen(run,meta)).not.toContain('shop-branch-overlay');});
});
