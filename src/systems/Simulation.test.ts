...
describe('simulation boundary',()=>{
 it('normalizes diagonal movement and freezes while paused',()=>{const sim=make();sim.state.invincible=true;for(let i=0;i<10;i++)sim.update(.1,{x:1,y:1},viewport);expect(Math.hypot(sim.state.player.x,sim.state.player.y)).toBeCloseTo(185);const before=sim.state.stageCombatTime;sim.pause();sim.update(1,{x:1,y:0},viewport);expect(sim.state.stageCombatTime).toBe(before);sim.resume();sim.update(.1,{x:0,y:0},viewport);expect(sim.state.stageCombatTime).toBeGreaterThan(before);});
